...
export const AddressForm = (props: Props) => {
  ...
  const [currentDistrictSelected, setCurrentDistrictSelected] = useState(
    props.area !== '',
  );

  // Handle Address
  const [addr, setAddr] = props.addr.includes('/nl/')
    ? useState({
  ...
  return (
    <>
      <FormControl isDisabled={!props.enabled}>
        <Text style={[styles.subTitle, styles.mv_10]}>收貨聯絡人</Text>
        <Input
          size="m"
          placeholder="聯絡人"
          value={props.name}
        ...
        <Text style={[styles.subTitle, styles.mv_10]}>聯絡電話</Text>
        <View flexDirection={'row'}>
          <Input
            ...
            isInvalid={
              !(
                parseInt(props.phone) != NaN &&
                props.phone.slice(0, 3).length > 0
              )
            }
            isDisabled={true}
            onChangeText={text =>
              props.setInput({
                ...props.input,
                phone: text + props.phone.slice(3),
              })
            }
          />
          <Input
            ...
            isInvalid={
              !(
                parseInt(props.phone) != NaN && props.phone.slice(3).length == 8
              )
            }
            onChangeText={text =>
              props.setInput({
                ...props.input,
                phone: props.phone.slice(0, 3) + text,
              })
            }
          />
        </View>
