...
export const PrescriptionPaymentPage = (props: any) => {
    ...
    const redirectPaypal = async () => {
        ...
        return { status: 'success', data: { 'nonce': '123', 'payerId': '123', 'email': 'a@a.com', "firstName": 'firstName', "lastName": 'lastName', "phone": '85251123122' } }
        ...
        try {
            ...
            const { nonce, payerId, email, firstName, lastName, phone } = await requestOneTimePayment(
                ...
                `${Config.PAYPAL}`
                , {
                    amount: `${fetchData.bill[0].totel_amount}`, // required
    ...
    return (
        <SafeAreaView style={[backgroundStyle, { flex: 1 }]}>
            <ScrollView contentInsetAdjustmentBehavior="automatic" style={{ backgroundColor: 'white', marginBottom: 2, marginLeft: 5 }}>
                ...
                {fetchData &&
                    <View>
                        <Text style={[styles.subTitle, { paddingHorizontal: 15, paddingTop: 15, }]}>藥單費用：${fetchData.bill[0].totel_amount}</Text>
                    </View>
                }
                 {submitStatus ?
