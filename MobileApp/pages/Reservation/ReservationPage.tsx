...
const checkPatient = async (id: string, token: string) => {
  try {
    const response = await fetch(
      `${Config.REACT_APP_API_SERVER}/patient/${id}`,
      {
        headers: {
...
export const ReservationPage = (props: any) => {
  ...
  const onSubmit = async (data: ReservationType) => {
    ...
    let patientStatus:
      | ReservationSubmitType1
      | ReservationSubmitType2
      | undefined = await checkPatient(data.idNumber, userToken);

    if (patientStatus?.message === 'Not Found') {
      props.navigation.navigate('上傳身份證明文件', {
        name: data.name,
        isNewPatient: true,
      });
            ...
            <View style={[styles.pageMargin, {paddingBottom: '15%'}]}>
              ...
              <View
                ...
                <Text style={styles.subTitle}>
                  暫定問診費用： $
                  {docData.video_diag_fee === 0
                    ? 0
                    : docData.video_diag_fee || 9999}
                </Text>
